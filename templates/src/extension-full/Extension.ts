...
export class SimpleTree implements ITree {
  ...
  place(location: Vector3, settings: TreeToolSettings): TreeBlockChangeData[] {
    ...
    const heightOffset =
      Math.floor(Math.random() * settings.randomHeightVariance) -
      settings.randomHeightVariance / 2;
...
const TreeTypes = [
  {
    name: "Oak",
    type: new SimpleTree(
      BlockPermutation.resolve(MinecraftBlockTypes.oakLog.id),
      createLeaf1Block("oak")
    ),
  },
  {
    name: "Spruce",
    type: new SimpleTree(
      BlockPermutation.resolve(MinecraftBlockTypes.spruceLog.id),
      createLeaf1Block("spruce")
    ),
  },
  {
    name: "Birch",
    type: new SimpleTree(
      BlockPermutation.resolve(MinecraftBlockTypes.birchLog.id),
      createLeaf1Block("birch")
    ),
  },
  {
    name: "Jungle",
    type: new SimpleTree(
      BlockPermutation.resolve(MinecraftBlockTypes.jungleLog.id),
      createLeaf1Block("jungle")
    ),
  },

  {
    name: "Acacia",
    type: new SimpleTree(
      BlockPermutation.resolve(MinecraftBlockTypes.acaciaLog.id),
      createLeaf2Block("acacia")
    ),
  },
  {
    name: "Dark Oak",
    type: new SimpleTree(
      BlockPermutation.resolve(MinecraftBlockTypes.darkOakLog.id),
      createLeaf2Block("dark_oak")
    ),
  },
];
...
function addToolSettingsPane(uiSession: IPlayerUISession, tool: IModalTool) {
  ...
  const onExecuteTool = (ray?: Ray) => {
    ...
    const targetBlock =
      ray != null
        ...
        : player.dimension.getBlock(
            uiSession.extensionContext.cursor.getPosition()
          );
    ...
    // Track changes
    uiSession.extensionContext.transactionManager.trackBlockChangeList(
      affectedBlocks.map((x) => x.location)
    );
    ...
    if (invalidBlockCount > 0) {
      uiSession.log.warning(
        `There were ${invalidBlockCount} invalid blocks while placing a tree!`
      );
    }
...
export function registerExtension() {
  registerEditorExtension(
    ...
    (uiSession: IPlayerUISession) => {
      uiSession.log.debug(
        `Initializing ${uiSession.extensionContext.extensionName} extension`
      );
    ...
    (uiSession: IPlayerUISession) => {
      uiSession.log.debug(
        `Shutting down ${uiSession.extensionContext.extensionName} extension`
      );
    },
